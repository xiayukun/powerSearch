...
axios
  ...
  .catch((error) => {
    if (
      ...
      error.response.headers["set-cookie"][0].includes("ASP.NET_SessionId")
    ) {
      ...
      if (
        ...
        axios
          .get(
            "http://www.langyuewy.com/Pay_Info.aspx?code=10000&state=STATE",
            {
          ...
          .then((response) => {
            ...
            console.log(
              `您好，${name}。\n您的余额为：${balance}元。\n您的电表为：${kwh}度。\n您的缴费记录为：\n${payList
                .map((i) => i.date + "通过" + i.type + "缴费" + i.money + "元")
                .join("\n")}`
            );
            debugger;
          });
